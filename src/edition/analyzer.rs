...
impl EditionAnalyzer {
    ...
    pub async fn analyze(&self, target_edition: Edition) -> Result<AnalysisReport> {
        ...
        self.check_edition_lints(target_edition, &mut report).await?;
        
        // Check for common patterns that need updating
        self.check_common_patterns(target_edition, &mut report).await?;
    ...
    async fn check_edition_lints(&self, _edition: Edition, report: &mut AnalysisReport) -> Result<()> {
    ...
    /// Check for common patterns that need updating
    async fn check_common_patterns(&self, target_edition: Edition, report: &mut AnalysisReport) -> Result<()> {
        match target_edition {
            ...
            Edition::Edition2021 => {
                ...
                report.warnings.push(
                    "Or patterns in matches are now available".to_string()
                );
            }
            Edition::Edition2024 => {
    ...
    fn add_suggestions(&self, target_edition: Edition, report: &mut AnalysisReport) {
        ...
        report.suggestions.push(
            "Review and test your code after migration".to_string()
        );
        
        if target_edition >= Edition::Edition2018 {
            report.suggestions.push(
                "Consider using `cargo fmt` to update code style".to_string()
            );
        }
    }
