...
impl EditionMigrator {
    ...
    pub async fn migrate(&self, target_edition: Edition, options: MigrationOptions) -> Result<MigrationResult> {
        ...
        self.run_cargo_fix(target_edition, &options, &mut result).await?;
    ...
    async fn create_backup(&self, result: &mut MigrationResult) -> Result<()> {
        ...
        result.messages.push("Backup created successfully".to_string());
    ...
    async fn run_cargo_fix(
        ...
        let cargo_path = which::which("cargo")
            .map_err(|_| Error::rust_not_found("cargo not found"))?;
        ...
        if !output.status.success() {
            ...
            result.messages.push("cargo fix completed successfully".to_string());
        }
    ...
    async fn update_manifest(&self, target_edition: Edition, result: &mut MigrationResult) -> Result<()> {
        let manifest_path = self.project_path.join("Cargo.toml");
    ...
    async fn run_tests(&self, result: &mut MigrationResult) -> Result<()> {
        ...
        if !output.status.success() {
            result.warnings.push("Tests failed after migration".to_string());
            result.status = MigrationStatus::PartialSuccess;
        } else {
            result.messages.push("All tests passed after migration".to_string());
        }
...
mod tests {
    ...
    async fn test_migrator_creation() {
        ...
        let migrator = EditionMigrator::new(temp_dir.path())
            .with_backup(temp_dir.path().join("backup"));
