...
pub async fn execute(path: Option<PathBuf>) -> Result<()> {
    ...
    println!();
    println!("{}", style("📚 Checking documentation coverage...").bold().yellow());
    match doc_coverage::check_documentation_coverage(&project_path).await {
        ...
        Err(e) => {
            println!("{}", style(format!("⚠️  Could not check documentation: {}", e)).yellow());
        }
    }
    
    // Check formatting
    println!();
    println!("{}", style("📝 Checking code formatting...").bold().yellow());
    match formatting::check_formatting(&project_path).await {
        Ok(format_result) => {
            println!("{}", format_result.report());
        }
        Err(e) => {
            println!("{}", style(format!("⚠️  Could not check formatting: {}", e)).yellow());
        }
    }
    ...
    match security::run_security_audit(&project_path).await {
        ...
        Err(e) => {
            println!("{}", style(format!("⚠️  Could not run security audit: {}", e)).yellow());
        }
    }
