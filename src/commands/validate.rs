...
use crate::{doc_coverage, formatting, security, validation::{RustValidator, Violation}, Result};
use console::style;
...
pub async fn execute(path: Option<PathBuf>, ai_report: bool, _compare_previous: bool) -> Result<()> {
...
async fn generate_ai_report(project_path: &PathBuf, violations: &[Violation]) -> Result<()> {
    ...
    for (i, violation) in violations.iter().take(50).enumerate() { // Limit to 50 for AI processing
        let code_snippet = get_code_snippet(&violation.file, violation.line).await
        ...
        let (suggested_fix, auto_fixable, priority) = match violation.violation_type {
            crate::validation::ViolationType::UnderscoreBandaid => {
                if violation.message.contains("parameter") {
                    ("Remove unused parameter or implement missing functionality".to_string(), false, 2)
                } else {
                    ("Replace `let _ =` with proper error handling using `?`".to_string(), true, 1)
                }
            }
            crate::validation::ViolationType::UnwrapInProduction => {
                ("Replace `.unwrap()` with `?` or proper error handling".to_string(), true, 1)
            }
            crate::validation::ViolationType::FileTooLarge => {
                ("Split file into smaller modules following single responsibility principle".to_string(), false, 4)
            }
            crate::validation::ViolationType::FunctionTooLarge => {
                ("Extract helper functions or split into smaller, focused functions".to_string(), false, 3)
            }
            _ => ("Review and fix according to Ferrous Forge standards".to_string(), false, 3),
        };
    ...
    let files_with_violations = violations.iter()
        .map(|v| &v.file)
