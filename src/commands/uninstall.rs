...
pub async fn execute(confirm: bool) -> Result<()> {
    ...
    println!("{}", style("🗑️  Uninstalling Ferrous Forge...").bold().red());
    ...
    println!("{}", style("✅ Ferrous Forge has been uninstalled").bold().green());
    println!();
...
fn remove_ferrous_forge_section(content: &str) -> String {
    ...
    for line in lines {
        ...
        if skip && (line.trim().is_empty() || line.starts_with('#') || line.contains("FERROUS_FORGE")) {
            continue;
        }
