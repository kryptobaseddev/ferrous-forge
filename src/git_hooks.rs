...
pub async fn install_git_hooks(project_path: &Path) -> Result<()> {
    ...
    if !git_dir.exists() {
        return Err(Error::validation("Not a git repository. Run 'git init' first."));
    }
...
async fn install_hook(hooks_dir: &Path, name: &str, content: &str) -> Result<()> {
    ...
    if hook_path.exists() {
        ...
        println!("  ⚠️  Backed up existing {} hook to {}", name, backup_path.display());
    }
...
pub async fn check_hooks_installed(project_path: &Path) -> Result<bool> {
    ...
    if pre_commit.exists() {
        let content = fs::read_to_string(&pre_commit)
            .await
            .unwrap_or_default();
