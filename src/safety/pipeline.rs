...
impl SafetyPipeline {
    ...
    pub async fn run_checks(&self, stage: PipelineStage) -> Result<SafetyReport> {
        ...
        if self.config.parallel_checks && checks.len() > 1 {
            ...
            self.run_checks_parallel(checks, &mut report, multi_progress.as_ref()).await?;
        } else {
            // Run checks sequentially
            self.run_checks_sequential(checks, &mut report, multi_progress.as_ref()).await?;
        }
    ...
    async fn run_checks_parallel(
        ...
        self.run_checks_sequential(checks, report, _multi_progress).await
    }
    
    /// Run a single safety check
    async fn run_single_check(&self, check_type: CheckType) -> Result<CheckResult> {
        let stage_config = self.config.get_stage_config(self.get_stage_for_check(check_type));
        let check_timeout = Duration::from_secs(stage_config.timeout_seconds);
    ...
    fn get_stage_for_check(&self, check_type: CheckType) -> PipelineStage {
        ...
        for stage in [PipelineStage::PreCommit, PipelineStage::PrePush, PipelineStage::Publish] {
